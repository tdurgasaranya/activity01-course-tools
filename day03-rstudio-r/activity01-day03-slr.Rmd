...
# Adding the values
result <- 2 + 5
# integer vector with values 1 through 9
int_vect <- 1:9
# character vector with letters "A" through "Q"
char_vect <- letters[1:17]
# Counting the number of items in char_vector occurring after the letter "D"
count <- sum(char_vect > "D")
```

 Provides a descriptive comment for each of the above tasks.
...
- The dimensions of the dataset are as below, number of rows are 1458 and 123 columns
...
### The following data chunk is updated with filtering for 2016 year data only

- Filter the data `hfi` data frame for year 2016, and
- Assigns the result to a data frame named `hfi_2016`.

```{r hfi-2016}
Link <- "https://www.openintro.org/data/csv/hfi.csv"

# Filtering for year 2016

Data <- read.csv(Link)
head(Data)
hfi_2016 <- filter(Data, year == 2016)
...
- Scatter plot is used below to demonstrate the numerical values distribution between the two variables.
...
#scatter plot
library(moments)
plot(Data$pf_score, Data$pf_expression_control, 
     main = "Scatter Plot: pf_score vs. pf_expression_control", 
     xlab = "pf_score", ylab = "pf_expression_control")


center <- mean(Data$pf_expression_control)  # Measure of center (mean)
spread <- sd(Data$pf_expression_control)    # Measure of spread (standard deviation)
shape <- skewness(Data$pf_expression_control)  # Measure of shape (skewness)
...
# Create a histogram for pf_Score
ggplot(data = Data, aes(x = Data$pf_score)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black") +
  labs(title = "Histogram of pf_Score", x = "pf_Score", y = "Frequency")

# Create a histogram for pf_expression
ggplot(data = Data, aes(x = Data$pf_expression_control)) +
  geom_histogram(binwidth = 1, fill = "green", color = "black") +
  labs(title = "Histogram of pf_expression", x = "pf_expression", y = "Frequency")
...
- I would prefer Histogram over scatter plot because of the efficient values display and distribution for a clear understanding.
...
# Create scatter plot with pf_expression_control as the predictor
plot(Data$pf_expression_control, Data$response_variable,
     main = "Scatter Plot: pf_expression_control vs. Response Variable",
     xlab = "pf_expression_control", ylab = "Response Variable")
...
- according to the linearity assumption and the scatter plot distributions the plot looks somewhat linear as the points are distributed in a symmetrical manner.
#### Challenge
...
```{r challenge}
cor = cor(Data$pf_score, Data$pf_expression_control,method="spearman")
print(cor)
```

- to use the correlation function to determine the linearity strength and by using the relation methods like pearson and spearman will define the extent of linearity between two numerical values.
...
(
  pf_expression_control = 0:10 )
# Use mutate on the data frame
pred_df <- pred_df %>% 
  mutate(
    pred_value = predict(slr_mod, newd_data = pred_df) %>% pull(.pred)
  )
